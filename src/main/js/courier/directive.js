// Builds a self-contained, JSON-serializable delivery directive.
// Each publisher entry is a sealed envelope — fully self-contained,
// no shared context. This allows future multi-target publishing
...
export const buildDirective = (pkg, ctx, {
  snapshot = false,
  publishers = [],
  npmTarball,
  releaseNotes,
  ...
  return {
    ...
    publishers: Object.fromEntries(
      publishers.map(n => [n, entry[n]?.(pkg, ctx, a) ?? {}])
    ),
  }
}
