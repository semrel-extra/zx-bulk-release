...
describe('steps.analyze', () => {
  ...
  test('resolvePkgVersion()', async () => {
    ...
    cases.forEach(([releaseType, prev, def, expected]) => {
      expect(resolvePkgVersion(releaseType, prev, def)).toBe(expected)
    })
  })

  test('getNextReleaseType returns major for breaking change', () => {
    expect(getNextReleaseType([{releaseType: 'patch'}, {releaseType: 'major'}])).toBe('major')
  })

  test('getNextReleaseType returns minor for feat', () => {
    expect(getNextReleaseType([{releaseType: 'patch'}, {releaseType: 'minor'}])).toBe('minor')
  })

  test('getNextReleaseType returns null for empty', () => {
    expect(getNextReleaseType([])).toBe(null)
  })

  test('getNextVersion with previous version', () => {
    expect(getNextVersion('patch', '1.0.0')).toBe('1.0.1')
    expect(getNextVersion('minor', '1.0.0')).toBe('1.1.0')
    expect(getNextVersion('major', '1.0.0')).toBe('2.0.0')
  })

  test('getNextVersion with pre-release suffix', () => {
    expect(getNextVersion('patch', '1.0.0', '1.0.0', '-snap.abc1234')).toBe('1.0.1-snap.abc1234')
  })

  test('getNextVersion without previous version uses default', () => {
    expect(getNextVersion('patch', undefined, '0.1.0')).toBe('0.1.0')
    expect(getNextVersion('patch', undefined)).toBe('1.0.0')
  })

  test('resolvePkgVersion returns null when no releaseType', () => {
    expect(resolvePkgVersion(null, '1.0.0', '0.0.0')).toBe('1.0.0')
    expect(resolvePkgVersion(null, undefined, '0.0.0')).toBe(null)
  })

  test('releaseSeverityOrder is major > minor > patch', () => {
    expect(releaseSeverityOrder).toEqual(['major', 'minor', 'patch'])
  })
  ...
  test('getSemanticChanges parses commits with rules', async () => {
    await within(async () => {
      ...
      expect(changes[0].releaseType).toBe('patch')
      expect(changes[1].releaseType).toBe('minor')
    })
  })
  ...
  test('analyze sets pkg fields', async () => {
    await within(async () => {
      ...
      expect(pkg.releaseType).toBe('patch')
      expect(pkg.version).toBe('1.0.1')
      expect(pkg.tag).toBeTruthy()
      expect(pkg.changes.length > 0).toBeTruthy()
    })
  })
  ...
  test('analyze with snapshot flag', async () => {
    await within(async () => {
      ...
      expect(pkg.version.includes('-snap.')).toBeTruthy()
      expect(pkg.preversion).toBeTruthy()
    })
  })
  ...
  test('analyze with no semantic changes', async () => {
    await within(async () => {
      ...
      expect(pkg.releaseType).toBe(null)
      expect(pkg.tag).toBe(null)
    })
  })
