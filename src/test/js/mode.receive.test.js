...
describe('modes.receive', () => {
  ...
  test('writes context with packages that have releases', async () => {
    await within(async () => {
      ...
      const {runReceive} = await import(/* @vite-ignore */ `../../main/js/post/modes/receive.js?t=${Date.now()}`)
      ...
      expect(await fs.pathExists(contextFile)).toBeTruthy()
  ...
  test('writes skip context when nothing to release', async () => {
    await within(async () => {
      ...
      const contextFile = path.resolve(cwd, 'zbr-context.json')
      const context = await fs.readJson(contextFile)
      expect(context.status).toBe('skip')
      expect(context.reason).toBe('nothing to release')
    })
  })
  ...
  test('dryRun prevents rebuild signal consumption', async () => {
    await within(async () => {
      ...
      expect(deleteTagCalls.length).toBe(0)
    })
  })
  ...
  test('rebuild trigger consumed when not dryRun', async () => {
    await within(async () => {
      ...
      const deleteTagCalls = mock.calls.filter(c => c.includes('git push origin :refs/tags'))
      expect(deleteTagCalls.length > 0).toBeTruthy()
    })
  })
  ...
  test('rebuild signal already consumed writes skip context', async () => {
    await within(async () => {
      ...
      const context = await fs.readJson(contextFile)
      expect(context.status).toBe('skip')
      expect(context.reason).toBe('rebuild claimed by another process')
    })
  })
  ...
  test('tagsFetched triggers re-analyze of packages without releaseType', async () => {
    await within(async () => {
      ...
      expect(reAnalyzeEvents.length > 0 || has(mock.calls, 'git fetch origin --tags --force')).toBeTruthy()
