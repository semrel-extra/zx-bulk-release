...
describe('modes.pack', () => {
  ...
  test('dryRun skips pack and publish', async () => {
    await within(async () => {
      ...
      const {runPack} = await import(/* @vite-ignore */ `../../main/js/post/modes/pack.js?t=${Date.now()}`)
      ...
      expect(has(mock.calls, 'echo build')).toBeTruthy()
      expect(has(mock.calls, 'echo test')).toBeTruthy()
      expect(has(mock.calls, 'npm publish')).toBeFalsy()
    })
  })
  ...
  test('skips packages with no releaseType after analyze', async () => {
    await within(async () => {
      ...
      expect(has(mock.calls, 'echo build')).toBeFalsy()
    })
  })
  ...
  test('publish=false skips publish but runs build and test', async () => {
    await within(async () => {
      ...
      expect(has(mock.calls, 'npm pack')).toBeFalsy()
    })
  })
  ...
  test('pack with context filter applies version/tag/channels from context', async () => {
    await within(async () => {
      ...
      expect(pkg.tag).toBe('v2.0.0')
      expect(pkg.contextChannels).toEqual(['npm'])
